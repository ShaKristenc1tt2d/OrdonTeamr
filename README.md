...
### 2015年11月06日   availdate.js RC版本正式发布：
* [新增] 整个项目基础原型


非常感谢流浪男版主带给我非常好的AUI布局框架，让我们的开发速度再一次的升华！但是，想更快的开发，更省代码吗？availdate.js能帮到你！
...
标签属性	属性描述
data-rule	必填，这个属性是整个验证插件的基础。
注意：需要验证的插件必须填写该属性，但除了指定data-sync属性以外。
data-nullmsg	选填，当表单为空的时候提示信息。
data-errmsg	选填，当表单验证失败的时候提示信息。
data-sucmsg	选填，当表单执行成功的时候提示信息。
data-sync	选填，同步验证，一般用于密码的验证。
比如，第一次输入密码之后，然后第二次密码需要和第一次输入密码进行同步验证。
...
*	描述：必填，不能为空。
例子：data-rule="*"
*6-16	描述：必填，不能为空，且必须是6到16个字符
例子：data-rule="*3-12"，data-rule="*5-10"。分别表示：必填，3到12个字符；必填，5到10个字符。
n	描述：必填，不能为空，且必须是正整数。
例子：data-rule="n"
n6-16	描述：必填，不能为空，且必须是6到16个字符
例子：data-rule="n3-12"
s	描述：必填，不能为空，且不能包含特殊字符的字符串
例子：data-rule="s"
s6-16	描述：必填，不能为空，且必须是6到16位的非包含特殊字符的字符串
例子：data-rule="s6-16"
p	描述：必填，不能为空，且必须是正确的邮政编码
例子：data-rule="p"
m	描述：必填，不能为空，且必须是正确的手机号码
例子：data-rule="m"
e	描述：必填，不能为空，且必须是电子邮箱
例子：data-rule="e"
url	描述：必填，不能为空，且必须是网址格式
例子：data-rule="url"
date	描述：必填，不能为空，且必须是日期格式
例子：data-rule="date"
zh	描述：必填，不能为空，且必须是中文
例子：data-rule="zh"
dword	描述：必填，不能为空，且必须填写双字节字符
例子：data-rule="dword"
money	描述：必填，不能为空，且必须是金额类型
例子：data-rule="money"
ipv4	描述：必填，不能为空，且必须是IPv4地址
例子：data-rule="ipv4"
ipv6	描述：必填，不能为空，且必须是IPv6地址
例子：data-rule="ipv6"
num	描述：必填，不能为空，必须是数值类型，例如正整数，正浮点数，小数。
例子：data-rule="num"
qq	描述：必填，不能为空，必须是qq号码
例子：data-rule="qq"

idcard                          描述：必填，不能为空，必须是身份证号码             例子：data-rule="idcard"

time                             描述：必填，不能为空，必须是时间格式号码，如：10:23:60             例子：data-rule="time"
